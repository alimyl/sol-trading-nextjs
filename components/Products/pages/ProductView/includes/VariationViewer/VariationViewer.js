...
export default function VariationViewer(props) {
    ...
    return (
        <React.Fragment>
            {
                variantMainFields?.length && variantMainFields.map(item => {
                    return (
                        <div key={item.id} className={`${productViewStyles["item"]} st-fs-14 pb-3`}>
                            {
                                item?.display_type === "rectangle_list" && <Rectangle data={item} />
                            }
                            {
                                item?.display_type === "radio_buttons" && <Radio data={item} />
                            }
                            {
                                item?.display_type === "dropdown" && <Dropdown data={item} />
                            }
                        </div>
                    )
                })
